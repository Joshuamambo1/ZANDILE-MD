...
cmd({ 
  ...
  reply("*_DATABASE UPDATE ☘𝙉3𝙩𝙆𝙞𝙣𝙜 𝙈𝘿☘-XMD RESTARTING NOW...🚀_*"); 
  await sleep(1500); 
  exec("pm2 restart all"); 
  reply("*☘𝙉3𝙩𝙆𝙞𝙣𝙜 𝙈𝘿☘ STARTED NOW...🚀_*"); 
});
...
cmd({
    pattern: "mode",
    desc: "Set bot mode to private, grouponly or public.",
    category: "settings",
    filename: __filename,
    ...
    if (!args[0]) {
        return reply(`📌 Current mode: *${config.MODE}*\n\nUsage: .mode private, .mode grouponly OR .mode public`);
    }
    ...
    if (modeArg === "private") {
        ...
        config.MODE = "grouponly";
        return reply("*_BOT MODE IS NOW SET TO GROUPONLY ✅_*.")
        const {exec} = require("child_process")
reply("*_DATABASE UPDATE N3tKing MD RESTARTING NOW...🚀_*")
await sleep(1500)
exec("pm2 restart all")
reply("*_N3tKing MD STARTED NOW...🚀_*");
    } else {
        return reply("❌ Invalid mode. Please use `.mode private, '.mode grouponly` or `.mode public`.");
    }
});
